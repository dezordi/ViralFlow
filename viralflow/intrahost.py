...
def __get_region(var_pos):
    '''
    get region name of a given var position for SARS-COV2 genome
    '''
    if int(var_pos) <= 265:
        region = '5UTR'
...
def __write_formated_bam_csv(bam_rc_file, depth_value):
    '''
    Write formatted BAM csv based on a '.fa.bc' file
    '''
    def __parse_depth_line(depth_line):
        '''
        '''
        # get depth
        depth = ":".join(depth_line.split(":", 2)[:2])
        depth = int(re.sub(r".*:", "", depth))
        # plus sign
        plus = ":".join(depth_line.split(":", 6)[:6])
        plus = int(re.sub(r".*:", "", plus))
        # minus sign
        minus = ":".join(depth_line.split(":", 7)[:7])
        minus = int(re.sub(r".*:", "", minus))

        return depth, plus, minus

    def __subtract_indel(depth, plus, minus, indel_depth, indel_plus,
                         indel_minus):
        '''
        '''
        new_depth = int(depth) - int(indel_depth)
        new_plus = int(plus) - int(indel_plus)
        new_minus = int(minus) - int(indel_minus)
        return new_depth, new_plus, new_minus

    # load bam input
    bc_file = open(bam_rc_file, 'r')
    # set bamd formated tsv
    bc_formated_output = open(bam_rc_file+'.fmt.tsv', 'w')
    output_csv_writer = csv.writer(bc_formated_output, delimiter='\t')
    output_csv_writer.writerow(['GENOME', 'POS', 'REGION', 'DEPTH', 'A_DEPTH',
                                'A_PLUS', 'A_MINUS', 'C_DEPTH', 'C_PLUS',
                                'C_MINUS', 'G_DEPTH', 'G_PLUS', 'G_MINUS',
                                'T_DEPTH', 'T_PLUS', 'T_MINUS', 'INDEL_DEPTH',
                                'INDEL_PLUS', 'INDEL_MINUS', 'INDEL'])
    out_list = []
    # load deletion dictionary
    del_dict = {'pos': [], 'deletion': [], 'del_depth': [],
                'del_depth_plus': [], 'del_depth_minus': []}
    ...
    for line in bam_rc_file_read:
        ...
        A_depth, A_plus, A_minus = __parse_depth_line(A_depth_line)

        C_depth_line = line[6].rstrip('\n')
        C_depth, C_plus, C_minus = __parse_depth_line(C_depth_line)

        G_depth_line = line[7].rstrip('\n')
        G_depth, G_plus, G_minus = __parse_depth_line(G_depth_line)

        T_depth_line = line[8].rstrip('\n')
        T_depth, T_plus, T_minus = __parse_depth_line(T_depth_line)

        major_depth_column = 0
        depth_dict = {A_depth: "A_depth", C_depth: "C_depth",
                      G_depth: "G_depth", T_depth: "T_Depth"}
        ...
        # bamreadcount output can have 11 or more columns, depending of number
        # of putative indels
        if len(line) > 11:
            ...
            if '+' in indel_seq:
                if depth_dict.get(max(depth_dict)) == 'A_depth':
                    A_depth, A_plus, A_minus = __subtract_indel(A_depth,
                                                                A_plus,
                                                                A_minus,
                                                                indel_depth,
                                                                indel_plus,
                    ...
                    C_depth, C_plus, C_minus = __subtract_indel(C_depth,
                                                                C_plus,
                                                                C_minus,
                                                                indel_depth,
                                                                indel_plus,
                    ...
                    G_depth, G_plus, G_minus = __subtract_indel(G_depth,
                                                                G_plus,
                                                                G_minus,
                                                                indel_depth,
                                                                indel_plus,
                    ...
                    T_depth, T_plus, T_minus = __subtract_indel(T_depth,
                                                                T_plus,
                                                                T_minus,
                                                                indel_depth,
                                                                indel_plus,
                ...
                if int(indel_depth) < depth_value:
                    ...
                    for i in range(0, del_length):
                        if i == 0:
                            ...
                            del_dict['del_depth_plus'].append(indel_plus)
                            del_dict['del_depth_minus'].append(indel_minus)
        region = __get_region(var_pos)

        if 'pos' in del_dict and int(line[1]) in del_dict['pos']:
            item_index = del_dict['pos'].index(int(line[1]))
            out_list.append([bam_rc_file, var_pos, region, int(var_depth)+int(del_dict['del_depth'][item_index]), A_depth, A_plus, A_minus, C_depth, C_plus, C_minus, G_depth, G_plus, G_minus,
                            T_depth, T_plus, T_minus, del_dict['del_depth'][item_index], del_dict['del_depth_plus'][item_index], del_dict['del_depth_minus'][item_index], del_dict['deletion'][item_index]])
        elif len(line) > 10 and '+' in indel_seq:
            out_list.append([bam_rc_file, var_pos, region, var_depth, A_depth, A_plus, A_minus, C_depth, C_plus, C_minus,
                            G_depth, G_plus, G_minus, T_depth, T_plus, T_minus, indel_depth, indel_plus, indel_minus, indel_seq])
        else:
            out_list.append([bam_rc_file, var_pos, region, var_depth, A_depth, A_plus, A_minus, C_depth,
                            C_plus, C_minus, G_depth, G_plus, G_minus, T_depth, T_plus, T_minus, 0, 0, 0, 'Na'])
    output_csv_writer.writerows(out_list)


def __write_minor_tsv(bam_rc_file, depth_value, per_limit):
    '''
    '''
    bam_readcount_formated = open(bam_rc_file+'.fmt.tsv', 'r')

    # load as dataframe
    df = pd.read_csv(bam_readcount_formated, sep='\t', header=0)
    # set conditions for selections
    conditions = [
        # A/T
        ...
        & ((df['G_PLUS']/df['G_MINUS'] > 0.05)
           & (df['G_MINUS']/df['G_PLUS'] > 0.05))
        & ((df['C_PLUS']/df['C_MINUS'] > 0.05) & (df['C_MINUS']/df['C_PLUS'] > 0.05)),
    ...
    # ???
    minors = ['A/T', 'A/C', 'A/G', 'A/INDEL', 'T/C', 'T/G', 'T/INDEL', 'C/G',
              'C/INDEL', 'G/INDEL', 'A/T/C', 'A/T/G', 'A/G/C', 'T/G/C', 'A/T/C/G']
    # (O o)
    df['PUTATIVE_MINOR'] = np.select(conditions, minors, default='False')
    # This all come downs to get putative minor
    df = df.loc[df['PUTATIVE_MINOR'] != "False"]
    # write csv
    df.to_csv(bam_rc_file+'.fmt.minors.tsv', sep='\t', index=False)


def __write_intrahost_tsv(bam_rc_file, alignment_file):
    '''
    ...
    output_tsv_writer.writerow(["GENOME", "POS", "REGION", "DEPTH", "A_DEPTH", "A_PLUS", "A_MINUS", "C_DEPTH", "C_PLUS", "C_MINUS", "G_DEPTH", "G_PLUS", "G_MINUS",
                                "T_DEPTH", "T_PLUS", "T_MINUS", "INDEL_DEPTH", "INDEL_PLUS", "INDEL_MINUS", "INDEL", "PUTATIVE_MINOR", "MAJOR", "MINOR", "MAJOR_DEPTH", "MINOR_DEPTH", "FREQUENCY"])
    out_list_lists = []
    output_short_tsv = open(bam_rc_file+'.intrahost.short.tsv', 'w')
    output_short_writer = csv.writer(output_short_tsv, delimiter='\t')
    output_short_writer.writerow(["GENOME", "POS", "REGION", "DEPTH",
                                  "MAJOR", "MINOR", "MAJOR_DEPTH", "MINOR_DEPTH", "FREQUENCY"])
    ...
    with open(bam_rc_file+'.fmt.minors.tsv', 'r') as minor_tsv:
        ...
        for line in minor_tsv_reader:
            ...
            if 'GENOME' not in line[0]:
                # For intrahost loci with 3 or more alleles, don't check frequency
                try:
                    frequency = "{:.2f}".format(
                        int(minor_depth)/int(line[3].rstrip('\n')))
                except:
                    frequency = 'CURATION'
                    lns_to_add = [line[i] for i in range(0, 21)]
                    out_list_lists.append([lns_to_add, major, minor,
                                          major_depth, minor_depth, frequency])
                    out_short_list_lists.append([line[0], line[1], line[2],
                                                line[3], major, minor,
                                                major_depth, minor_depth,
                                                frequency])
    ...
    for record in alignment:
        ...
        if record.id != 'NC_045512.2':
            record = SeqRecord(Seq(new_seq), id=record.id
                               + '_minor', description='')
...
def get_instrahost_tsv(bam_rc_file, alignment_file,
                       per_limit=0.05, depth_value=100):
    '''
    process bam readout to get intragost tsv file
    Parameters
    ----------
    bam_rc_file:<path>
        bam-readcount output file

    alignment_file:<path>
        Alignment file against reference genome (ONLY WUHAM SARS-COV-2 REF)

    per_limit:<float> (default = 0.05)
        Percentage limit of depth Base / Total Base to consider putative minor

    depth_number:<int> (default = 100)
        Minimum depth value to consider as minor variant
    '''
    __write_formated_bam_csv(bam_rc_file, depth_value)
    __write_minor_tsv(bam_rc_file, depth_value, per_limit)
    __write_intrahost_tsv(bam_rc_file, alignment_file)
